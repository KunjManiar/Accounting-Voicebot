...
class Invoice(object):
    ...
    def get_all_invoice(cls):
        if session['conditions'] != {}:
            ...
            session['conditions'] = {}
            return [cls(**elm) for elm in Database.find(InvoiceConstants.INVOICE_COLLECTION, x)]
        else:
            print(InvoiceConstants.INVOICE_COLLECTION)
            print(Database.find(InvoiceConstants.INVOICE_COLLECTION, {}))
            return [cls(**elm) for elm in Database.find(InvoiceConstants.INVOICE_COLLECTION, {})]

    @classmethod
    def get_by_no(cls,no):
        x = Database.find_one(InvoiceConstants.INVOICE_COLLECTION,{"no":no})
        if x is not None:
            return cls(**x)
        else:
            return None
    ...
    def get_by_id(cls,id):
        x = Database.find_one(InvoiceConstants.INVOICE_COLLECTION,{"_id":id})
        if x is not None:
            return cls(**x)
        else:
            return None
...
#     Database.update('invoices',{"_id" : invoice._id}, { "$set" : {"total_amount" : invoice.total_amount}})
#
#